...
def load_some_data(total_samples= 500, val_size=50):
    ...
    # Slice away unneeded samples
    X_in = X_in[:, :total_samples]
    Y_in = Y_in[:, :total_samples]
    y_in = y_in[:total_samples]
...
def save_MX_data(f=2, d=3, val_size=50, total_samples= 500, small_data=False):
    ...
    if small_data == True:
        X_tr, Y_tr, y_tr, X_val, Y_val, y_val, X_te, Y_te, y_te  = load_some_data(total_samples, val_size)
    else:
    ...
    if small_data == True: # Small data used for testing, then we need X_tr_clean
        n = X_tr_clean.shape[1]
        X_ims = np.transpose(X_tr_clean.reshape((32, 32, 3, n), order='F'), (1, 0, 2, 3))
